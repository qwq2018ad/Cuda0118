...
__global__ void computeHammingDistances(
    unsigned long long* bitText,   // 文字的位元串
    unsigned long long bitReference, // 參考字串的位元串
    int* distances,               // 輸出距離
    int numStrings,               // 總字串數量
    int parameterL,               // 參數 L
    int numberSymbolInULL,        // 每個 ULL 中的符號數
    unsigned long long filterUselessBit, // 過濾無效位元的遮罩
    unsigned long long clearWitnessBit,  // 過濾 Witness 位元的遮罩
    unsigned long long keepWitnessBit    // 保留 Witness 位元的遮罩
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= numStrings) return;

    int aux1 = numberSymbolInULL - (idx % numberSymbolInULL);
    int aux2 = idx / numberSymbolInULL;
    unsigned long long bitString = 0x0ULL;

    if (aux1 >= parameterL) {
        bitString = (bitText[aux2] >> ((aux1 - parameterL) * 8)) & filterUselessBit;
    }
    else {
        bitString = ((bitText[aux2] << ((parameterL - aux1) * 8)) |
            (bitText[aux2 + 1] >> ((numberSymbolInULL - parameterL + aux1) * 8))) & filterUselessBit;
    }

    // 計算 Hamming 距離
    unsigned long long diff = ((bitString ^ bitReference) + clearWitnessBit) & keepWitnessBit;
    //distances[idx] = __popcll(diff); // CUDA 提供的 popcount 操作
    distances[idx]=ComputeHD_64(diff);
}
void constructReferenceTreeGPU(
    unsigned long long* bitText,  // 位元串陣列
    int lengthText,               // 文字總長度
    int parameterL,               // 參數 L
    int numberInputStrings        // 總字串數量
) {
    int numBlocks = (numberInputStrings + MAX_BLOCK_SIZE - 1) / MAX_BLOCK_SIZE;
    int* distances; // Hamming 距離的結果

    // 分配 GPU 記憶體
    unsigned long long* d_bitText;
    int* d_distances;
    cudaMalloc((void**)&d_bitText, sizeof(unsigned long long) * lengthText);
    cudaMalloc((void**)&d_distances, sizeof(int) * numberInputStrings);

    // 將資料拷貝到 GPU
    cudaMemcpy(d_bitText, bitText, sizeof(unsigned long long) * lengthText, cudaMemcpyHostToDevice);

    // 初始化參數
    unsigned long long bitReference = 0x0ULL;
    unsigned long long filterUselessBit = 0x0ULL /* 過濾無效位元的遮罩 */;
    unsigned long long clearWitnessBit = 0x0ULL/* 過濾 Witness 位元的遮罩 */;
    unsigned long long keepWitnessBit = 0x0ULL/* 保留 Witness 位元的遮罩 */;
    int numberSymbolInULL = 1 /* 每個符號的位元數 */;

    // 啟動 CUDA 核心函數
    computeHammingDistances << <numBlocks, MAX_BLOCK_SIZE >> > (
        d_bitText, bitReference, d_distances, numberInputStrings,
        parameterL, numberSymbolInULL, filterUselessBit, clearWitnessBit, keepWitnessBit
        );

    // 將結果拷貝回主機
    distances = (int*)malloc(sizeof(int) * numberInputStrings);
    cudaMemcpy(distances, d_distances, sizeof(int) * numberInputStrings, cudaMemcpyDeviceToHost);

    // TODO: 基於 distances 進行分類處理
